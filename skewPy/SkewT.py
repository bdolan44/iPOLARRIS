...
def wind_components(speed, direction, met_angle= 1):
    "given a wind speed and direction, break it into u and v components"
    u = -1.0 *speed*np.sin(np.pi*direction/180.0)
    v = -1.0 *speed*np.cos(np.pi*direction/180.0)
    return u, v


def wind_speed_and_direction(u, v, units='m/s'):
    if units == 'm/s':
        multiplier = 2.2
    elif units == 'mph':
        multiplier = 1.0
        ...
        multiplier = 1.15
    wind_speed = multiplier * np.sqrt(u ** 2 + v ** 2)
    wind_direction = 180.0 * np.arctan2(-u, -v) / np.pi
    wind_direction[wind_direction < 0] += 360.0  # just to wrap them around
...
# Copyright (c) 2013 Thomas Chubb 
    ...
    def __init__(self, filename=None, data=None, fmt='UWYO', station_name=None, parcel_mixing_depth=50.0, 
                                calculate_parcel=True, surface_pressure=None):
...
#	let's try to interpolate the parcel stuff onto the environmental stuff
    ...
    def surface_parcel(self, mixdepth=100, pres_s=None): # added pres_s so can just change for diff regions
