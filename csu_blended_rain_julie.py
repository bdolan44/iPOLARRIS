...
def calc_rain_zr(dz, a=0.017, b=0.714):
    """
    dz = Reflectivity (dBZ), returns rainfall rate (mm h^-1)
    Form Z = a * R**b
    """
    return a*(linearize(dz)**b)


def calc_rain_nexrad(dz, thresh_nexrad=53.0, a=0.017, b=0.714):
    """
    dz = Reflectivity (dBZ)
...
def calc_blended_rain(dz, zdr, kdp, r_z_a, r_z_b, r_kdp_a, r_kdp_b,
        fit_a, fit_b, r_kdp_zdr_a, r_kdp_zdr_b, r_kdp_zdr_c, r_z_zdr_a, r_z_zdr_b, r_z_zdr_c, band='S',
        thresh_dz=38.0, thresh_zdr=0.25, thresh_kdp=0.2,
        thresh_frac_ice=0.1, thresh_nexrad=53.0,method='cr1995',ice_flag=False):
    ...
    print np.ma.is_masked(dz)
    print np.ma.is_masked(zdr)
...
def calc_zh_zv(dbz, zdr):
    Zh = linearize(dbz)
    return Zh, Zh / linearize(zdr)  # Horizontal and Vertical component of reflectivity
...
def unlinearize(Zh,Zv):
    Zh = 10.*np.log10(Zh)
    Zv = 10.*np.log10(Zv)
    return Zh, Zv

def csu_hidro_rain(
        dz, zdr, kdp, r_z_a, r_z_b, r_kdp_a, r_kdp_b,
        r_kdp_zdr_a, r_kdp_zdr_b, r_kdp_zdr_c, r_z_zdr_a, r_z_zdr_b, r_z_zdr_c,band='S', fhc=None, 
        thresh_dz=35.0, thresh_zdr=0.25, thresh_kdp=0.2, thresh_nexrad=53.0):
    """
    This algorithm ingests polarimetric radar data and computes rain rate,
    ...
    if band == 'S':
        ...
        cond_meth_4 = np.logical_and(cond_rain, cond_not_kdp_not_zdr) # was 4a
